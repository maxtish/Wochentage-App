...
export const ImageAndText: React.FC = () => {
  ...
  const allWords = useSelector((state: IState) => state.stateImageAndText.allWords);
  ...
  const onPress = (item: string) => {
    const newColor = correctWord === item ? isGoodColorButton : isBadColorButton;
  ...
  return (
    <View style={styles.container}>
      <Progress.Bar
        ...
        progress={(count + 1) / allWords.length}
        width={200}
        height={20}
        borderRadius={50}
      />
      ...
      <View style={styles.wrapperButons}>
        {shuffledButtonNameArr.map((item, index) => (
          <Pressable
            key={index}
            style={[styles.buttons, { backgroundColor: colorButtons[item] || defaultColorButton }]}
        ...
        <Pressable style={[styles.buttons, styles.buttonsReset]} onPress={() => dispatch(allWordsResetCount())}>
